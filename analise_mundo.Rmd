...
As estimativas das populações utilizadas foram obtidas no <a href='https://data.worldbank.org/indicator/SP.POP.TOTL'>The World Bank</a>.
...
covid <- left_join(x = covid, y = dia_ant, by = c("Country_Region", "date")) %>%
  mutate(
    ant_Confirmed = replace_na(ant_Confirmed, 0),
...
world <- left_join(
  x = world %>% filter(name_long != "Antarctica"),
...
world <- left_join(x = world, y = pop_2019, by = c("iso_a3" = "Country Code")) %>% #### OK
  ...
  select(-pop_est2) %>%
  mutate(
    Confirmed_pop = (Confirmed / pop_est) * 100000,
    Deaths_pop = (Deaths / pop_est) * 100000,
    Treatment_pop = ((Confirmed - Recovered) / pop_est) * 100000
  ) %>% 
  mutate(Treatment_pop = case_when(
    IND_INFO == "<span style='color:DarkRed'>Sem Informação</span>" ~ NA_real_, # Sem info de recuperação
    IND_INFO == "<span style='color:DarkGreen'>Atualizado</span>" ~ Treatment_pop, # atualizado
    TRUE ~ NA_real_
  ))

## Trazendo info sobre população para os dados covid
covid <- left_join(
  x = covid, y = world %>% st_drop_geometry() %>% select(name_long, continent, pop_est),
...
d <- covid$date %>% max() # data-base mais recente
semana_anterior <- covid %>% filter(date <= d & date>=d-13 & date<d-6) # penultima semana
ultima_semana <- covid %>% filter(date <= d & date>=d-13 & date>=d-6) # última semana

semana_anterior <- semana_anterior %>% group_by(Country_Region) %>% 
  summarise(Total_Confirmed_ant=sum(New_Confirmed), # Total de novos casos na penúltima semana
            Total_Deaths_ant=sum(New_Deaths),
            New_Confirmed_ant=mean(New_Confirmed),
            New_Deaths_ant=mean(New_Deaths)) %>% 
  select(Country_Region, New_Confirmed_ant, New_Deaths_ant, Total_Confirmed_ant, Total_Deaths_ant)

ultima_semana <- ultima_semana %>% group_by(Country_Region) %>% 
  summarise(Total_Confirmed=sum(New_Confirmed), # Total de novos casos na última semana
            Total_Deaths=sum(New_Deaths),
            New_Confirmed=mean(New_Confirmed),
            New_Deaths=mean(New_Deaths)) %>%
  select(Country_Region, New_Confirmed, New_Deaths, Total_Confirmed, Total_Deaths)

acomp_paises <- left_join(x=ultima_semana, y=semana_anterior, by="Country_Region") %>% 
  mutate(var_casos=ifelse(New_Confirmed_ant==0, NA, ((New_Confirmed-New_Confirmed_ant)/New_Confirmed_ant)*100),
         var_casos=ifelse(Total_Confirmed<100,NA,var_casos), # Pelo menos 20 obitos nas duas semanas
         var_casos=ifelse(Total_Confirmed_ant<100,NA,var_casos),
         var_obitos=ifelse(New_Deaths_ant==0, NA, ((New_Deaths-New_Deaths_ant)/New_Deaths_ant)*100),
         var_obitos=ifelse(Total_Deaths<20,NA,var_obitos), # Pelo menos 20 obitos nas duas semanas
         var_obitos=ifelse(Total_Deaths_ant<20,NA,var_obitos)) 

world_var_casos <- left_join(x=world, y=acomp_paises, by=c("name_long"="Country_Region"))
...
world_var_casos <- as(world_var_casos, Class="Spatial")
```
...
renderLeaflet({
  ...
  leaflet(data = world) %>%
    ...
    addPolygons(
      weight = 0.1, fillColor = ~ greens(Confirmed / 1000),
      color = "green", fillOpacity = 0.9,
      smoothFactor = 0.5,
      popup = paste0(
        world$name_long, ": ",
        format(world$Confirmed,
          decimal.mark = ",",
          big.mark = ".", small.mark = "."
        ), "<br>",
        "População: ", round(world$pop_est / 1000000, 2), " Mi"
      )
    ) %>%
    addLegend(
      position = "bottomright", pal = greensNA, values = ~ Confirmed / 1000,
      title = "Casos Confirmados<br>(em milhares)", na.label = "",
      opacity = 1
    ) %>%
    setView(lat = 15, lng = 10, zoom = 2)
})
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
renderLeaflet({
  greens <- colorNumeric("Greens", domain = world$Confirmed_pop)
  greensNA <- colorNumeric("Greens", domain = world$Confirmed_pop, na.color = rgb(0, 0, 0, 0))
  leaflet(data = world) %>%
    addTiles("http://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}.png",
      attribution = paste(
        '&copy; <a href="http://openstreetmap.org">OpenStreetMap</a> contributors',
    ...
    addPolygons(
      weight = 0.1, fillColor = ~ greens(Confirmed_pop),
      color = "green", fillOpacity = 0.9,
      smoothFactor = 0.5,
      popup = paste0(
        world$name_long, ": ",
        format(round(world$Confirmed_pop, 2),
    ...
    addLegend(
      position = "bottomright", pal = greensNA, values = ~Confirmed_pop,
...
O mapa abaixo mostra a **variação da média de novos casos dos últimos 7 dias com relação à media dos 7 dias antecedentes a eles**. Países que obtiveram **menos de 100 novos casos** nos últimos 7 dias ou nos 7 dias anteriores a esses foram **desconsiderados** da análise.

```{r echo=FALSE, message=FALSE, warning=FALSE}
renderLeaflet({
  
  rc1 <- colorRampPalette(colors = c("dodgerblue4", "white"), 
                          space = "Lab")(Mod(as.integer(min(world_var_casos$var_casos, na.rm = T))))

  rc2 <- colorRampPalette(colors = c("white", "firebrick4"), 
                          space = "Lab")(Mod(as.integer(max(world_var_casos$var_casos, na.rm = T))))

  rampcols <- c(rc1, rc2)
  
  greens = colorNumeric(rampcols, domain = world_var_casos$var_casos)
  greensNA <- colorNumeric(rampcols, domain = world_var_casos$var_casos, na.color=rgb(0,0,0,0))
  leaflet(data = world_var_casos) %>%
  addTiles('http://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}.png',
           attribution = paste(
             '&copy; <a href="http://openstreetmap.org">OpenStreetMap</a> contributors',
             '&copy; <a href="http://cartodb.com/attributions">CartoDB</a>'
                    ...
                    color = "green",fillOpacity = 0.9,
                    smoothFactor = 0.5,
                    popup = paste0(world_var_casos$name_long,": ", 
                                   round(world_var_casos$var_casos,2), "%","<br>",
                                   "População: ", round(world_var_casos$pop_est/1000000,2)," Mi")) %>% 
...
renderLeaflet({
  ...
  leaflet(data = world) %>%
    ...
    addPolygons(
      weight = 0.1, fillColor = ~ reds(Deaths),
    ...
    addLegend(
      position = "bottomright", pal = redsNA, values = ~Deaths,
...
renderLeaflet({
  ...
  leaflet(data = world) %>%
    ...
    addPolygons(
      weight = 0.1, fillColor = ~ reds(Deaths_pop),
      color = "reds", fillOpacity = 0.9,
      smoothFactor = 0.5,
      popup = paste0(
        world$name_long, ": ",
    ...
    addLegend(
      position = "bottomright", pal = redsNA, values = ~Deaths_pop,
      title = "Óbitos<br>(por 100 mil<br>habitantes)", na.label = "", opacity = 1
    ) %>%
    setView(lat = 15, lng = 10, zoom = 2)
})
```
...
O mapa abaixo mostra a **variação da média de novos óbitos dos últimos 7 dias com relação à media dos 7 dias antecedentes a eles**. Países que obtiveram **menos de 20 novos óbitos** nos últimos 7 dias ou nos 7 dias anteriores a esses foram **desconsiderados** da análise.

```{r echo=FALSE, message=FALSE, warning=FALSE}
renderLeaflet({
  
  rc1 <- colorRampPalette(colors = c("dodgerblue4", "white"), 
                          space = "Lab")(Mod(as.integer(min(world_var_casos$var_obitos, na.rm = T))))

  rc2 <- colorRampPalette(colors = c("white", "firebrick4"), 
                          space = "Lab")(Mod(as.integer(max(world_var_casos$var_obitos, na.rm = T))))

  rampcols <- c(rc1, rc2)
  
  greens = colorNumeric(rampcols, domain = world_var_casos$var_obitos)
  greensNA <- colorNumeric(rampcols, domain = world_var_casos$var_obitos, na.color=rgb(0,0,0,0))
  leaflet(data = world_var_casos) %>%
  addTiles('http://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}.png',
           attribution = paste(
             '&copy; <a href="http://openstreetmap.org">OpenStreetMap</a> contributors',
             '&copy; <a href="http://cartodb.com/attributions">CartoDB</a>'
                    ...
                    smoothFactor = 0.5,
                    popup = paste0(world_var_casos$name_long,": ", 
                                   round(world_var_casos$var_obitos,2), "%","<br>",
                                   "População: ", round(world_var_casos$pop_est/1000000,2)," Mi")) %>% 
...
renderLeaflet({
  ...
  redsNA <- colorNumeric("YlOrRd",
    domain = 100 * world_4$Deaths / world_4$Confirmed,
  ...
  leaflet(data = world_4) %>%
    ...
    addPolygons(
      weight = 0.1, fillColor = ~ reds(100 * Deaths / Confirmed),
      color = "reds", fillOpacity = 0.9,
      smoothFactor = 0.5,
      popup = paste0(
        world_4$name_long, ": ",
        ...
        "População: ", round(world_4$pop_est / 1000000, 2), " Mi"
      )
    ) %>%
    addLegend(
      position = "bottomright", pal = redsNA, values = ~ 100 * Deaths / Confirmed,
...
As informações sobre pacientes recuperados não são divulgadas por todos os países e, para outros, estão desatualizadas e correspondem a contagens de semanas ou até meses atrás. Por conta disso, foi criado um indicador que estima se as informações recentes sobre pacientes recuperados no Repositório da Universidade John Hopkins são atuais com base nas atualizações dos valores nos últimos 15 dias. Apenas os países em que a taxa calculada foi feita utilizando dados <span style="color:DarkGreen">**atualizados**</span> foram considerados. Os demais foram desconsiderados por possuírem dados possivelmente <span style="color:DarkOrange">**desatualizados**</span> ou por <span style="color:DarkRed">**não haver informação**</span> disponível sobre pacientes recuperados.
...
renderLeaflet({
  ...
  leaflet(data = world) %>%
    ...
    addPolygons(
      weight = 0.1, fillColor = ~ reds(Treatment_pop),
      color = "reds", fillOpacity = 0.9,
      smoothFactor = 0.5,
      popup = paste0(
        world$name_long, ": ",
    ...
    addLegend(
      position = "bottomright", pal = redsNA, values = ~Treatment_pop,
      title = "Pessoas em<br>Tratamento<br>(por 100 mil<br>habitantes)", na.label = "", opacity = 1
    ) %>%
    setView(lat = 15, lng = 10, zoom = 2)
})
```
...
renderDataTable({
  tabela <-
    ...
    mutate(
      ...
      Confirmed_pop = format(round(Confirmed_pop, 2),
        decimal.mark = ".", big.mark = ",",
        small.mark = ","
      ),
      Deaths_pop = format(round(Deaths_pop, 2),
        decimal.mark = ".", big.mark = ",",
        small.mark = ","
      ),
      Treatment_pop = format(round(Treatment_pop, 2),
        decimal.mark = ".", big.mark = ",",
        small.mark = ","
      ),
      pop_est = format(pop_est,
        decimal.mark = ".", big.mark = ",",
        small.mark = ","
      )
    ) %>%
    select(
      name_long, Confirmed, Confirmed_pop, Deaths, Deaths_pop, Tax_Let,
      Treatment_pop, pop_est
    ) %>%
    datatable(
      colnames = c("País", "Casos Acumulados", "Casos (Por 100 Mil Hab.)", "Óbitos Acumulados", "Óbitos (Por 100 Mil Hab.)", "Taxa de Letalidade", "Pacientes em Tratamento (Por 100 Mil Hab.)", "População Estimada"),
...
```{r}
covid_week <- covid %>%
  mutate(semanaEpi = epiweek(date))

covid_week$semanaEpi2 <- paste0(lubridate::year(covid_week$date),"-",formatC(covid_week$semanaEpi, width=2, flag="0"))

covid_week <- covid_week %>% 
  select(-semanaEpi) %>% 
  rename(semanaEpi=semanaEpi2) %>% 
  mutate(semanaEpi=case_when(date=="2021-01-01" ~ "2020-53",
                             date=="2021-01-02" ~ "2020-53",
                             TRUE ~ semanaEpi)) %>% 
...
covid_cont_week <- covid_cont %>%
  mutate(semanaEpi = epiweek(date))

covid_cont_week$semanaEpi2 <- paste0(lubridate::year(covid_cont_week$date),"-",formatC(covid_cont_week$semanaEpi, width=2, flag="0"))

covid_cont_week <- covid_cont_week %>% 
  select(-semanaEpi) %>% 
  rename(semanaEpi=semanaEpi2) %>% 
  mutate(semanaEpi=case_when(date=="2021-01-01" ~ "2020-53",
                             date=="2021-01-02" ~ "2020-53",
...
covid_world_week <- covid_world %>%
  mutate(semanaEpi = epiweek(date))

covid_world_week$semanaEpi2 <- paste0(lubridate::year(covid_world_week$date),"-",formatC(covid_world_week$semanaEpi, width=2, flag="0"))

covid_world_week <- covid_world_week %>% 
  select(-semanaEpi) %>% 
  rename(semanaEpi=semanaEpi2) %>% 
  mutate(semanaEpi=case_when(date=="2021-01-01" ~ "2020-53",
                             date=="2021-01-02" ~ "2020-53",
...
```{r}
darktheme <- theme_minimal() + theme(
  plot.title = element_text(color = "#c8c8c8"),
...
tags$div(selectizeInput(
  "paises",
  label = "Escolha até 6 Países:",
  choices = sort(unique(covid$Country_Region)),
  selected = (covid %>% filter(date == max(date)) %>% arrange(desc(Confirmed)))$Country_Region[1:2],
...
Compare os Países escolhidos com relação aos:
...
renderPlotly({
  d <- covid %>%
    filter(date == max(date)) %>%
    filter(Country_Region %in% input$paises)
  ...
  ggplotly(ggplot(data = d, aes(
    ...
    coord_flip() +
    xlab("") +
    ylab("Casos Acumulados (em milhões)") +
    darktheme +
...
renderPlotly({
  ...
  ggplotly(ggplot(data = d, aes(x = date, y = Confirmed / 1000000, color = Country_Region)) +
    geom_line(size = 0.8) +
    geom_point(size = .8, aes(text = paste0(format(date, "%d %b %Y"), ": ", format(round(Confirmed / 1000000, 2),
      decimal.mark = ",", big.mark = ".",
      small.mark = "."
    )))) +
    xlab("") +
...
renderPlotly({
  ...
  ggplotly(ggplot(data = d, aes(x = date, y = New_Confirmed, color = Country_Region)) +
    geom_line(size = 0.8) +
    geom_point(size = .8, aes(text = paste0(format(date, "%d %b %Y"), ": ", format(round(New_Confirmed, 2),
      decimal.mark = ",", big.mark = ".",
      small.mark = "."
    )))) +
    xlab("") +
...
renderPlotly({
  ...
  ggplotly(ggplot(data = d, aes(x = semanaEpi, y = New_Confirmed / 1000, color = Country_Region)) +
    geom_line(size = 0.8) +
    geom_point(size = .8, aes(text = paste0(semanaEpi, ": ", format(round(New_Confirmed / 1000, 2),
      decimal.mark = ",", big.mark = ".",
      small.mark = "."
    )))) +
    ...
    scale_color_hue(l = 40) + 
      scale_x_discrete(breaks=c("2020-04","2021-01"),
...
renderPlotly({
  d <- covid %>%
    ...
    filter(Country_Region %in% input$paises)
  ggplotly(ggplot(data = d, aes(
    x = reorder(Country_Region, -Confirmed / pop_est),
    y = 100000 * Confirmed / pop_est, fill = Country_Region,
    text = paste0(
      Country_Region, ": ",
      format(100000 * Confirmed / pop_est,
        decimal.mark = ",", big.mark = ".",
        small.mark = "."
      )
    )
    ...
    darktheme +
    theme(legend.position = "none") +
    ggtitle("Casos Acumulados por 100 Mil Habitantes") +
    scale_fill_hue(l = 40), tooltip = "text")
})
```
...
renderPlotly({
  d <- covid %>% filter(Country_Region %in% input$paises)
  ggplotly(ggplot(data = d, aes(
    x = date, y = 100000 * Confirmed / pop_est,
    color = Country_Region
  )) +
    geom_line(size = 0.8) +
    geom_point(size = .8, aes(text = paste0(format(date, "%d %b %Y"), ": ", format(round(100000 * Confirmed / pop_est, 2),
      decimal.mark = ",", big.mark = ".",
      small.mark = "."
    )))) +
    darktheme +
    xlab("") +
    ylab("Casos (por 100 mil habitantes)") +
    ggtitle("Evolução: Casos Acumulados por 100 Mil Habitantes") +
    labs(color = "") +
    scale_color_hue(l = 40), tooltip = "text") %>% layout(hovermode = "x unified")
})
```
...
renderPlotly({
  ...
  ggplotly(ggplot(data = d, aes(x = date, y = Deaths / 1000, color = Country_Region)) +
    geom_line(size = 0.8) +
    geom_point(size = .8, aes(text = paste0(format(date, "%d %b %Y"), ": ", format(round(Deaths / 1000, 2),
      decimal.mark = ",", big.mark = ".",
      small.mark = "."
    )))) +
    darktheme +
...
renderPlotly({
  ...
  ggplotly(ggplot(
    ...
    geom_line(size = 0.8) +
    geom_point(size = .8, aes(text = paste0(format(date, "%d %b %Y"), ": ", format(round(New_Deaths, 2),
      decimal.mark = ",", big.mark = ".",
      small.mark = "."
    )))) +
    darktheme +
...
renderPlotly({
  ...
  ggplotly(ggplot(
    ...
    geom_line(size = 0.8) +
    geom_point(size = .8, aes(text = paste0(semanaEpi, ": ", format(round(New_Deaths, 2),
      decimal.mark = ",", big.mark = ".",
      small.mark = "."
    )))) +
    darktheme +
...
renderPlotly({
  d <- covid %>%
    ...
    filter(Country_Region %in% input$paises)
  ggplotly(ggplot(data = d, aes(
    x = reorder(Country_Region, -Deaths / pop_est),
    y = 100000 * Deaths / pop_est, fill = Country_Region,
    text = paste0(
      Country_Region, ": ",
      format(round(100000 * Deaths / pop_est, 2),
        decimal.mark = ",", big.mark = ".",
        small.mark = "."
      )
    )
    ...
    darktheme +
    theme(legend.position = "none") +
    ggtitle("Óbitos Acumulados por 100 Mil Habitantes") +
    scale_fill_hue(l = 40), tooltip = "text")
})
```
...
renderPlotly({
  ...
  ggplotly(ggplot(
    data = d,
    aes(x = date, y = 100000 * Deaths / pop_est, color = Country_Region)
  ) +
    geom_line(size = 0.8) +
    geom_point(size = .8, aes(text = paste0(format(date, "%d %b %Y"), ": ", format(round(100000 * Deaths / pop_est, 2),
      decimal.mark = ",", big.mark = ".",
      small.mark = "."
    )))) +
    darktheme +
    xlab("") +
    ylab("Óbitos (por 100 mil habitantes)") +
    ggtitle("Evolução: Óbitos Acumulados (por 100 mil habitantes)") +
    labs(color = "") +
    scale_color_hue(l = 40), tooltip = "text") %>% layout(hovermode = "x unified")
})
```
...
renderPlotly({
  ggplotly(ggplot(
    data = covid_cont %>% filter(continent == "Africa" | continent == "Asia" | continent == "Europe" | continent == "North America" | continent == "Oceania" | continent == "South America"),
    aes(x = date, y = Confirmed / 1000, color = continent)
  ) +
    geom_line(size = 1) +
    geom_point(size = .8, aes(text = paste0(format(date, "%d %b %Y"), ": ", format(round(Confirmed / 1000, 2),
      decimal.mark = ",", big.mark = ".",
      small.mark = "."
    )))) +
    darktheme +
...
renderPlotly({
  ggplotly(ggplot(
    data = covid_cont %>% filter(continent == "Africa" | continent == "Asia" | continent == "Europe" | continent == "North America" | continent == "Oceania" | continent == "South America"),
    aes(x = date, y = 100000 * Confirmed / pop_est, color = continent)
  ) +
    geom_line(size = 1) +
    geom_point(size = .8, aes(text = paste0(format(date, "%d %b %Y"), ": ", format(round(100000 * Confirmed / pop_est, 2),
      decimal.mark = ",", big.mark = ".",
...
renderPlotly({
  ggplotly(ggplot(
    data = covid_cont %>% filter(continent == "Africa" | continent == "Asia" | continent == "Europe" | continent == "North America" | continent == "Oceania" | continent == "South America"),
    aes(x = date, y = 100000 * Deaths / pop_est, color = continent)
  ) +
    geom_line(size = 1) +
    geom_point(size = .8, aes(text = paste0(format(date, "%d %b %Y"), ": ", format(round(100000 * Deaths / pop_est, 2),
      decimal.mark = ",", big.mark = ".",
...
renderPlotly({
  ggplotly(ggplot(
    data = covid_cont_week %>% filter(continent == "Africa" | continent == "Asia" | continent == "Europe" | continent == "North America" | continent == "Oceania" | continent == "South America"),
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
renderPlotly({
  ggplotly(ggplot(data = covid_world, aes(x = date, y = 100000 * Confirmed / pop_est)) +
    geom_line(size = 1, color = "darkgreen") +
    geom_point(size = .8, color = "darkgreen", aes(text = paste0(format(date, "%d %b %Y"), ": ", format(round(100000 * Confirmed / pop_est, 2),
      decimal.mark = ",", big.mark = ".",
      small.mark = "."
    )))) +
    darktheme +
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
renderPlotly({
  ggplotly(ggplot(data = covid_world, aes(x = date, y = New_Confirmed / 1000)) +
    geom_point(color = "darkgreen", aes(text = paste0(format(date, "%d %b %Y"), ": ", round(New_Confirmed / 1000, 2)))) +
    geom_line(size = 0.8, color = "darkgreen") +
    darktheme +
    ggtitle("") +
    xlab("") +
...
renderPlotly({
  ggplotly(ggplot(data = covid_world_week, aes(x = semanaEpi, y = New_Confirmed / 1000)) +
           geom_bar(stat = "identity", fill = "darkgreen", aes(text = paste0(semanaEpi, ": ", round(New_Confirmed / 1000, 2)))) +
           darktheme +
           ggtitle("") +
           xlab("") +
           ylab("Casos Novos (em milhares)") +
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
renderPlotly({
  ggplotly(ggplot(data = covid_world, aes(x = date, y = 100000 * Deaths / pop_est)) +
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
renderPlotly({
  ggplotly(ggplot(data = covid_world, aes(x = date, y = New_Deaths / 1000)) +
    geom_point(color = "darkred", aes(text = paste0(format(date, "%d %b %Y"), ": ", round(New_Deaths / 1000, 2)))) +
    geom_line(size = 0.8, color = "darkred") +
    darktheme +
    ggtitle("") +
    xlab("") +
...
renderPlotly({
  ggplotly(ggplot(data = covid_world_week, aes(x = semanaEpi, y = New_Deaths / 1000)) +
           geom_bar(stat = "identity", fill = "darkred", aes(text = paste0(semanaEpi, ": ", round(New_Deaths / 1000, 2)))) +
           darktheme +
           ggtitle("") +
           xlab("") +
           ylab("Óbitos (em milhares)") +
