...
dia_ant <- covid %>% mutate(date=date+1) %>% 
...
world <- left_join(x=world, y=pop_2019, by=c("iso_a3"="Country Code")) %>% #### OK
  ...
  mutate(Confirmed_pop=(Confirmed/pop_est)*100000,
...
renderLeaflet({
  ...
  addLegend(position = "bottomright", pal = redsNA, values = ~Deaths_pop, 
            title="Óbitos<br>(por 100 mil<br>habitantes)", na.label = "", opacity = 1) %>% setView(lat=15, lng=10, zoom=2)
})
```
...
renderLeaflet({
  ...
  addLegend(position = "bottomright", pal = redsNA, values = ~Treatment_pop, 
            title="Pessoas em<br>Tratamento<br>(por 100 mil<br>habitantes)", na.label = "", opacity = 1) %>% setView(lat=15, lng=10, zoom=2)
})
```
...
tags$div(selectizeInput(
  'paises', label = 'Escolha Até 6 Países:',
...
renderPlotly({
  ...
  ggplotly(ggplot(data = d, aes(x=reorder(Country_Region, -Confirmed/pop_est), 
                                ...
                                text=paste0(Country_Region,": ",
                                            format(100000*Confirmed/pop_est, decimal.mark=",", big.mark=".",
                                          small.mark=".")))) + 
  geom_bar(stat="identity") + coord_flip() + xlab("") + 
  ylab("Casos por 100 Mil Habitantes") + darktheme + theme(legend.position = "none") +
  ggtitle("Casos Acumulados por 100 Mil Habitantes") + scale_fill_hue(l=40), tooltip = "text")
})
```
...
renderPlotly({
  d <- covid %>% filter(Country_Region %in% input$paises)
  ggplotly(ggplot(data = d, aes(x=date, y=100000*Confirmed/pop_est,
                                color=Country_Region)) + 
  geom_line(size=0.8) + 
    geom_point(size=.8, aes(text=paste0(format(date, "%d %b %Y"),": ",format(round(100000*Confirmed/pop_est,2), decimal.mark=",", big.mark=".",
                                          small.mark=".")))) + 
    darktheme + xlab("") + 
...
renderPlotly({
  ...
  ggplotly(ggplot(data = d, aes(x=reorder(Country_Region, -Deaths/pop_est), 
                                ...
                                text=paste0(Country_Region,": ",
                                            format(round(100000*Deaths/pop_est,2), decimal.mark=",", big.mark=".",
                                          small.mark=".")))) + 
  geom_bar(stat="identity") + coord_flip() + xlab("") + 
  ylab("Óbitos por 100 Mil Habitantes") + darktheme + theme(legend.position = "none") +
  ggtitle("Óbitos Acumulados por 100 Mil Habitantes") + scale_fill_hue(l=40), tooltip = "text")
})
```
...
renderPlotly({
  ...
  geom_line(size=0.8) + 
    geom_point(size=.8, aes(text=paste0(format(date, "%d %b %Y"),": ",format(round(100000*Deaths/pop_est,2), decimal.mark=",", big.mark=".",
                                          small.mark=".")))) + 
    darktheme + xlab("") + 
...
renderPlotly({
  ...
  geom_line(size=1, color="darkgreen") + 
    geom_point(size=.8, color="darkgreen", aes(text=paste0(format(date, "%d %b %Y"),": ",format(round(100000*Confirmed/pop_est,2), decimal.mark=",", big.mark=".",
                                          small.mark=".")))) + 
    darktheme +
  xlab("") + 
  ylab("Casos Confirmados (por 
