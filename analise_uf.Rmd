...
renderLeaflet({
  ...
  leaflet(data = dat, options = leafletOptions(zoomControl = FALSE)) %>%
    ...
    addLegend(position = "bottomright", pal = redsNA, values = ~obitos_100k, 
              title="Óbitos (por 100 mil<br>habitantes)", na.label = "", opacity = 1) %>% setView(lat=st_coordinates(dat)[,2] %>% mean(), lng=st_coordinates(dat)[,1] %>% mean(), zoom=5)
})
```
...
renderDataTable({
  tabela <- 
    mun_munis %>% filter(estado==input$estado) %>% st_as_sf() %>% st_drop_geometry() %>% 
    ...
    mutate(Tax_Let=
      ...
      TRUE ~ paste0(ifelse(casosAcumulado==0,0,round(100*obitosAcumulado/casosAcumulado,2)),"%")
           ...
           pop_2019=format(pop_2019, decimal.mark=".", big.mark=",", 
                          small.mark=",")) %>% 
    select(name_muni, casosAcumulado, casos_100k, obitosAcumulado, obitos_100k, Tax_Let, 
           pop_2019) %>% 
    datatable(
...
```{r}
mun <- mun %>% filter(populacaoTCU2019>=100000) %>% 
...
tags$div(selectizeInput(
  'munic', label = 'Escolha Municípios:',
  choices = sort(unique(mun$mun_est)), 
  selected = (mun %>% filter(data==max(data)) %>% arrange(desc(casosAcumulado)))$mun_est[1:6], 
  width='1000px', multiple=T, options = list(maxItems = 6)), 
  style="display:inline-block")
```

Escolha até 6 municípios (com mais de 100 mil habitantes) de todo o Brasil e os compare com relação aos:

##### Casos

```{r echo=FALSE, message=FALSE, warning=FALSE}
renderPlotly({
  d <- mun %>% filter(data==max(data)) %>% filter(mun_est %in% input$munic)
  
  ggplotly(ggplot(data = d, aes(x=reorder(mun_est, -casosAcumulado), 
